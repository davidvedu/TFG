...
module.exports = {
  ...
  eliminarVehiculo: async (req, res) => {
    try {
      ...
      // Verificar si el vehículo está asociado a algún anuncio activo
      const anunciosAsociados = await Anuncio.findAll({
        where: {
          matricula,
          estado: {
            [Op.ne]: 'vendido'
      ...
      if (anunciosAsociados.length > 0) {
        return res.status(400).json({
          error: 'No se puede eliminar el vehículo porque tiene anuncios activos asociados',
          anuncios: anunciosAsociados
        });
      }
  ...
  obtenerVehiculosPorMarca: async (req, res) => {
    try {
      ...
      // Obtener los IDs de los modelos
      const modeloIds = modelos.map(modelo => modelo.id);

      // Obtener vehículos que tengan esos modelos
      const vehiculos = await Vehiculo.findAll({
        where: {
          modelo_id: {
            [Op.in]: modeloIds
  ...
  obtenerVehiculosPorModelo: async (req, res) => {
    try {
      const { modeloId } = req.params;

      // Verificar si el modelo existe
      const modeloExiste = await Modelo.findByPk(modeloId);
      if (!modeloExiste) {
        return res.status(404).json({ error: 'Modelo no encontrado' });
      }

      // Obtener vehículos del modelo
      const vehiculos = await Vehiculo.findAll({
      ...
      console.error('Error al obtener vehículos por modelo:', error);
      res.status(500).json({ error: 'Error al obtener los vehículos por modelo' });
    }
  },

  // Obtener tipos de transmisión disponibles
  obtenerTiposTransmision: async (req, res) => {
    try {
      // Obtener los tipos de transmisión desde el modelo
      const tiposTransmision = Vehiculo.rawAttributes.transmision.values;
      res.json(tiposTransmision);
    } catch (error) {
      console.error('Error al obtener tipos de transmisión:', error);
      res.status(500).json({ error: 'Error al obtener los tipos de transmisión' });
    }
  },

  // Obtener tipos de tracción disponibles
  obtenerTiposTraccion: async (req, res) => {
    try {
      // Obtener los tipos de tracción desde el modelo
      const tiposTraccion = Vehiculo.rawAttributes.traccion.values;
      res.json(tiposTraccion);
    } catch (error) {
      console.error('Error al obtener tipos de tracción:', error);
      res.status(500).json({ error: 'Error al obtener los tipos de tracción' });
    }
  },

  // Obtener estados de vehículo disponibles
  obtenerEstadosVehiculo: async (req, res) => {
    try {
      // Obtener los estados de vehículo desde el modelo
      const estadosVehiculo = Vehiculo.rawAttributes.estado_vehiculo.values;
      res.json(estadosVehiculo);
    } catch (error) {
      console.error('Error al obtener estados de vehículo:', error);
      res.status(500).json({ error: 'Error al obtener los estados de vehículo' });
    }
  },
